...
var SSE_CONNECTED = Buffer.from(':' + new Array(2049).join(' ') + '\n\n', 'utf8'),
    SSE_SET_RETRY = Buffer.from('retry: 2000\n\n', 'utf8'),
    SSE_HEARTBEAT = Buffer.from(':\n\n', 'utf8'),
    SSE_HB_INTERVAL = 10000;

impress.sse.mixin = function(application) {

  application.sse.nextConnectionId = 1; // counter to be used as connection identifier
  ...
  application.Client.prototype.sseConnect = function() {
    ...
    if (client.eventChannel && !client.res.headersSent) {
      ...
      if (user && !api.common.inArray(channel, user.login)) {
