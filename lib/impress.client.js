...
var Client = function(application, req, res) {
  ...
  api.common.extend(client.parameters, client.query);
  ...
  if (client.ip) {
    ...
    client.local = api.common.inArray(api.impress.localIPs, client.ip);
  }
...
Client.prototype.destroySession = function() {
  ...
  if (application && client.session) {
    ...
    application.security.deletePersistentSession(client.sid, api.common.emptyness);
    client.sid = null;
    client.user = null;
    client.session = null;
  }
...
Client.prototype.parseCookies = function() {
  ...
  if (cookies) cookies.split(';').forEach(function(cookie) {
    ...
    client.cookies[api.common.trim(parts[0])] = api.common.trim(parts[1] || '');
  });
};
...
//
Client.prototype.setCookie = function(name, value, host = this.req.headers.host, httpOnly) {
...
//   host - host name
//
Client.prototype.deleteCookie = function(name, host = this.req.headers.host) {
  if (api.net.isIP(host) === 0) host = '.' + host;
  this.preparedCookies.push(name + impress.COOKIES_EXPIRES + host);
};
...
Client.prototype.dispatch = function() {
  ...
  } else if (code === impress.waf.ACCESS_DENIED) {
    client.error(403);
  } else if (code === impress.waf.ACCESS_LIMITED) {
    client.error(429);
...
Client.prototype.saveSession = function(callback) {
  ...
  if (
    ...
    client.session &&
    (client.sessionCreated || client.sessionModified)
  ) {
...
Client.prototype.processing = function() {
  ...
  if (cache && (client.startTime < cache.expireTime)) client.sendCache(cache);
    ...
    if (client.typeExt === 'sse') client.eventChannel = null;
      ...
      if (client.typeExt !== 'ws' && !client.res.headersSent) {
    ...
    if (client.ext === 'rpc' && application.rpc) {
      application.rpc.initialize(client);
    }

    // Execute handlers
    api.common.eachSeries(handlers, function(handler, cb) {
...
Client.prototype.extCases.rpc = function(client, application) {
  if (application.rpc) {
    application.rpc.finalize(client);
  } else {
    if (application.websocket) {
      application.websocket.initialize(client);
      application.websocket.finalize(client);
...
Client.prototype.extCases.json = function(client, application) {
  if (client.context.data) {
    client.end(JSON.stringify(client.context.data));
  } else client.error(400);
};
...
Client.prototype.basicAuth = function() {
  ...
  if (authBase64) {
    ...
    var auth = Buffer.from(authBase64, 'base64');
...
Client.prototype.defaultContentType = function() {
  ...
  var allowOrigin = api.common.getByPath(
    application.config,
  ...
  if (fileCache) {
    ...
    if (exists) {
      ...
      if (client.execPath !== '/' && handler !== 'meta') {
        ...
        application.cache.watch('/server' + client.execPath);
      } else {
...
Client.prototype.runScript = function(handler, fileName, callback) {
  ...
  application.createScript(fileName, function(err, fn) {
    if (err) {
      ...
      client.access = api.common.extend(client.access, fn);
      client.calculateAccess();
      callback();
    } else if (client.access.allowed && client.application) {
      var fnWrapper = function() {
        if (fn && fn.meta) {
          var vd = api.definition.validate(
            client.parameters,
            fn.meta,
            'parameters',
            true
            ...
            if (fn.length === 2) {
              ...
              fn(client, function(result, errorCode, headers) {
                ...
                if (headers && !client.res.headersSent) {
                  ...
                  } else for (var headerName in headers) {
...
Client.prototype.static = function(onNotServed) {
  ...
  else relPath += api.common.stripTrailingSlash(client.path);
...
Client.prototype.staticCache = function(relPath, onNotServed) {
  ...
  var cache = application.cache.static[relPath],
      isFlag = typeof(cache) === 'number',
      cached = cache && !isFlag;
  if (cached) client.buffer(cache);
  else if (isFlag) onNotServed();
  return cached;
};
...
Client.prototype.serveStatic = function(relPath, onNotServed) {
  ...
  function serveFile(err, stats) {
    if (err) {
      ...
      application.cache.watch(api.path.dirname(relPath));
      var isIndex = api.common.endsWith(relPath, '/index.html');
...
Client.prototype.buffer = function(cache) {
  ...
  if (sinceTime && api.common.isTimeEqual(sinceTime, cache.stats.mtime)) {
    client.error(304);
