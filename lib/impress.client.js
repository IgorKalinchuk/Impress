...
Client.prototype.processing = function() {
  ...
  if (cache && (client.startTime < cache.expireTime)) client.sendCache(cache);
    ...
    // Initialize long connections: SSE, WebSocket, Impress RPC, JSTP
    ...
    if (client.ext === 'jstp' && application.jstp) {
      application.jstp.initialize(client);
    }
...
// JSTP finalization
//
Client.prototype.extCases.jstp = function(client, application) {
  if (application.jstp) {
    application.jstp.finalize(client);
  } else {
    if (application.websocket) {
      application.websocket.initialize(client);
      application.websocket.finalize(client);
